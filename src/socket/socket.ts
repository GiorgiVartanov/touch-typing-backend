  ...
  StartListeners = (socket: Socket) => {
    ...
    socket.on("join_match", async (match_id: string, callback: () => void) => {
      ...
      if (uid && this.user_to_match[uid] === undefined) {
        ...
        let keys = Object.keys(this.matches[match_id].players).filter(
          (key) =>
            this.matches[match_id].players[key].wants_to_see_result === true
        );
        if (this.matches[match_id].has_started !== true) {
          ...
          if (keys.length === this.matches[match_id].user_limit) {
            ...
            const text: string = await generateText(
              this.matches[match_id].request
            ); //text will be sent to all the users after the game starts.
            this.matches[match_id].text = text;
            this.insertTimer(
              match_id,
              this.matches[match_id].time_limit * 1000
            ); //add a timer.
            this.SendMessage(
              "match_modified",
              this.getSocketIdsFromUids(keys),
              this.matches
            ); //Can be changed to taking a specific game and modifying games[gid] only... probably should do that...
          }
          //probably should only update the specific game, instead of the whole games object.
          this.SendMessage(
            "matches_modified",
            Object.values(this.users).filter(
              (id) => this.user_to_match[id] === undefined
            ),
            this.matches
          );
        } else {
    ...
    socket.on("leave_match", (match_id: string, callback: () => void) => {
      ...
      if (
        ...
        this.matches[match_id] &&
        (this.matches[match_id].players[uid] ||
          this.matches[match_id].spectators[uid])
      ) {
    ...
    socket.on(
      ...
      (match_id: string, currentWordIndex: number, callback: () => void) => {
        ...
        if (
          uid &&
          this.matches[match_id] &&
          this.matches[match_id].players[uid] !== undefined
        ) {
    ...
    socket.on(
      "user_finish",
      async (user_wpm: number, match_id: string, callback: () => {}) => {
        callback();
        ...
        if (this.matches[match_id].active_players === 0) {
          ...
          if (match_has_finished === true) {
            ...
            Object.keys(this.matches[match_id].players).forEach((uid) => {
              //players
              if (this.user_to_match[uid] !== undefined)
                delete this.user_to_match[uid];
              delete this.matches[match_id].players[uid];
            });
            //removing spectators
          ...
          if (match_has_finished === true) {
            ...
            match_to_store.players = this.sortTheDictionary(
              match_to_store.players
            );
            ...
            this.SendMessage(
              "match_finished",
              this.getSocketIdsFromUids(keys),
              { matches: this.matches, match_id: res._id }
            );
...
const generateText = async (
  ...
  if (req.type == "FakeWords") {
    ...
    return data
      .map((el: { _id: string; word: string; count: number }) => el.word)
      .join(" ");
  }
  return "";
};
